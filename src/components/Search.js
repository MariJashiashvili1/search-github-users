...
const Search = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // if user is gonna be empty string then i'm not gonna do anything
    if (user) {
  ...
  return <section className='section'>
    <Wrapper className='section-center'>
      {/* if the error property is true, only then display error wrappers */}
      {error.show &&
        <ErrorWrapper>
          <p>{error.msg}</p>
        </ErrorWrapper>
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-control">
          ...
          {/* if requests is biiger than zero, only then show button */}
